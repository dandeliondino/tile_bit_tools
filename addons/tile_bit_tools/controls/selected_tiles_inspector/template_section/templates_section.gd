...
var TerrainPicker := preload("res://addons/tile_bit_tools/controls/selected_tiles_inspector/template_section/terrain_picker.tscn")
var SelectedTag := preload("res://addons/tile_bit_tools/controls/selected_tiles_inspector/template_section/selected_tag.tscn")
...
var selected_template : TemplateBitData
...
@onready var context : Context = inspector_manager.get_current_context()
...
func _ready() -> void:
	if !is_instance_valid(inspector_manager):
		_print.debug("inspector_manager invalid in templates_section::ready()")
		return
	
	inspector_manager.templates_updated.connect(_on_templates_updated)
	tiles_manager.reset_requested.connect(_on_reset_requested)
...
func _update_tags_popup() -> void:
	tags_popup.clear()
	var item_list := inspector_manager.template_loader.get_tags_item_list(true, true, selected_tags)
...
func _update_templates_option_button() -> void:	
	...
	var templates_list := inspector_manager.template_loader.get_templates_item_list(selected_tags)
...
func _on_template_selected(index : int) -> void:
	...
	selected_template = inspector_manager.template_loader.get_template(selected_template_id)
...
func _update_template_panel() -> void:
	...
	if selected_template:
		stylebox = context.base_control.get_theme_stylebox("sub_inspector_property_bg0", "Editor")
	else:
...
func _update_terrain_set_option_button() -> void:
	var item_list := []
	if selected_template:
		item_list = context.get_terrain_sets_item_list(selected_template.terrain_mode)
...
func _add_terrain_picker(index : int) -> Control:
	...
	terrain_picker.setup(context, index, selected_template.get_terrain_color(index))
...
func _request_preview() -> void:
	...
	tiles_manager.apply_template_terrains(selected_template, selected_terrain_set, terrain_mapping)
...
func _on_reset_requested() -> void:
	_print.debug("_on_reset_requested from templates section")
	_update_templates_option_button()
