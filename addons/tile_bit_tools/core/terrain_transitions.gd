...
# base_tiles[terrain_set][terrain_id] = [TileLocation, ...]
var base_tiles : Dictionary


func get_base_tiles(terrain_set : int, terrain_id : int) -> Array:
...
func setup_from_bit_data(bit_data : BitData, source_id := -1) -> void:
	transition_sets = {}
	base_tiles = {}
#	var mode := bit_data.terrain_mode
	...
	for coords in bit_data.get_coordinates_list():
		...
		var transition_set := _get_transition_set(tile_bits, bit_data.terrain_set, bit_data.mode)
...
func setup_from_tile_set(tile_set : TileSet) -> void:
	...
	for source_id in range(tile_set.get_source_count()):
		...
		for tile_index in range(source.get_tiles_count()):
			...
			var bits_dict := {}
			for bit in BitData.CellNeighborsByMode[mode]:
				bits_dict[bit] = tile_data.get_terrain_peering_bit(bit)
				
			var tile_bits := BitData.TileBits.new(terrain_set, tile_data.terrain, bits_dict)
			var tile_location := TransitionSet.TileLocation.new(source_id, coords)
			if tile_bits.is_base_tile():
				_add_base_tile(tile_bits, tile_location)
				continue
			
			var transition_set := _get_transition_set(tile_bits, terrain_set, mode)
			transition_set.add_tile(tile_bits, tile_location)
...
func _get_transition_set(tile_bits : BitData.TileBits, terrain_set : int, mode : TileSet.TerrainMode) -> TransitionSet:
	...
	if transition_sets.has(transition_set_id):
		...
		transition_set = TransitionSet.new(terrain_set, mode, tile_bits.terrain_id, tile_bits.peering_terrain_ids)
